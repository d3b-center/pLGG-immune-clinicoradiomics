...
option_list <- list(
  ...
  make_option(c("--germline_file"), type = "character", default = NULL,
...
lgg_risk_groups <- lgg_risk_groups %>%
  dplyr::inner_join(dplyr::select(histology_file, any_of(c('cohort_participant_id', 
                                                           'Kids_First_Biospecimen_ID'))), 
                    by = c("SubjectID" = "cohort_participant_id"))

# merge risk groups with lgg_germline
lgg_germline <- lgg_germline %>%
  dplyr::inner_join(dplyr::select(lgg_risk_groups, any_of(c('Risk Group', 
                                                            'Kids_First_Biospecimen_ID'))), 
...
lgg_germline <- lgg_germline %>%
  ...
  dplyr::mutate(risk_group = factor(risk_group, levels = c('Low', 'Medium', 'High'), 
                                    ordered = FALSE)) %>%
...
for (i in 1:length(genes_of_interest)){
  ...
  fish_res <- fisher.test(x = xtabs(lgg_germline_counts$y ~ lgg_germline_counts$risk_group + lgg_germline_counts$`get(genes_of_interest[i])`), 
                          hybrid = T)
  result[[i]] <- fish_res
