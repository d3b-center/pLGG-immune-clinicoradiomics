...
option_list <- list(
  ...
  make_option(c("--tmb_file"), type = "character",
              help = "TMB file (.tsv)"),
...
anno_file <- anno_file %>%
  dplyr::mutate(cluster_assigned = as.factor(cluster_assigned),
                cluster_assigned = relevel(cluster_assigned, ref = 3),
                molecular_subtype = as.factor(molecular_subtype),
                molecular_subtype = relevel(molecular_subtype, ref = 'LGG, wildtype')) %>%
  dplyr::group_by(molecular_subtype, cluster_assigned) %>%
  dplyr::mutate(y = n())

modp <- glm(y ~ molecular_subtype*cluster_assigned, data = anno_file, family = poisson)
...
anno_file_ca <- anno_file %>% 
  dplyr::select(c('molecular_subtype', 'cluster_assigned', 'y')) %>%
  unique(by = 'molecular_subtype') %>%
  dplyr::mutate(cluster_assigned = paste0('cluster ', cluster_assigned)) %>%
  tidyr::pivot_wider(names_from = 'cluster_assigned', values_from = 'y') %>%
  tibble::column_to_rownames('molecular_subtype')
...
histology_DNA <- tmb_file %>%
  inner_join(histology %>% dplyr::select(Kids_First_Biospecimen_ID, composition, sample_id), 
...
p <- ggplot(tmb_df, aes(x = cluster_assigned, y = log(tmb + 1), fill = cluster_assigned)) +
  geom_violin() +
